...
class ValuePlusAdvantage extends tf.layers.Layer {
  ...
  call(input) { return input[1].sub(input[1].mean()) }
...
class NNModel {
  ...
  async train(inputs, targets) {
    return await this.model.fit( tf.tensor(inputs), tf.tensor(targets),{verbose:0}).then(history => history)
  }
}
...
export default class ExpDoubleDuelQPlayer {
  constructor(
    ...
    loss_value = -1.0, learning_rate = 0.01, training = true, random_move_prob = 0.9999, 
    random_move_decrease = 0.95, batch_size = 50, pre_training_games = 50
  ) {
